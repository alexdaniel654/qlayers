...
![Layers, R~2~ ^\*^ maps and layer profiles measured using `3DQLayers` for a subject with healthy renal function and a subject with impaired renal function. Shading around lines shows the 95% confidence interval. \label{fig:egfr_gradients}](gradients.png){ width=100% }
